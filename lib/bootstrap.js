    ...
    async.series([
        ...
        function(done){
            async.parallel([
                ...
                // break code.
                function(cb){
                    var speakerAdapterConfig = system.getConfig().system.speakerAdapter;
                    if(speakerAdapterConfig === null) {
                        logger.warn('No speaker adapter set. You will not have any audio output.');
                        return cb();
                    }
                    var SpeakerAdapter = speakerAdapterConfig.module;
                    system.speaker.registerSpeakerAdapter(new SpeakerAdapter(system, speakerAdapterConfig.options), cb);
                },
