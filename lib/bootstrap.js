    ...
    async.series([
        ...
        function(done){
            async.parallel([
                ...
                function(cb){
                    var adapter = new DefaultTextToSpeechAdapter(system);
                    adapter.initialize(function(err){
                        if(err){
                            return cb(err);
                        }
                        system.speaker.setTextToSpeechAdapter(adapter);
                    ...
                    system.bus.on('task:created', function(task) {
                        ...
                        if(system.runtimeHelper.profile.hasActiveProfile()){
                            ...
                            system.runtimeHelper.registerTask(newTask, function(err){
                                ...
                                system.notificationService.push('success', utils.format('Task %s registered', newTask.id));
                            });
                        }
                    });
