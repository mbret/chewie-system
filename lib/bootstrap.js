...
module.exports = function(system, logger, cb){
    ...
    async.series([
         ...
         */
        function(done){
            system.sequelize = new Sequelize('database', 'admin', null, system.getConfig().database.connexion);

            system.sequelize.models = {};
            system.sequelize.models.User = require(CORE_DIR + '/models/user')(system.sequelize, system);

            // create table
            system.sequelize.models.User.sync({force: true})
                .then(function () {
                    system.sequelize.models.User.initAdmin()
                        .then(function(){
                            done();
                        })
                        .catch(done);
