                ...
                return PluginsDao.findByIdOrName(pluginId)
                    .then(function(plugin) {
                        ...
                        if(!plugin.hasModule(moduleName)) {
                            throw new Errors.BadRequestError("The module " + moduleName + " does not exist for plugin " + plugin.name);
                        }

                        task.userId = user.id;
                        task.pluginId = plugin.id;
                        task.moduleId = plugin.getModuleByName(moduleName).id;
                                ...
                                TasksDao.create(task)
                                    ...
                                    .catch(function(err) {
                                        if(err.name = "SequelizeUniqueConstraintError") {
                                            err = new Errors.BadRequestError("The task name already exist");
                                        }
                                        return reject(err);
                                    });
                            // }
