...
module.exports = function(server, router){
    ...
    router.post('/users/:user/plugins/:plugin/modules/:module/tasks', function(req, res){
        ...
        UserDao.findByIdOrUsername(userId)
            .then(function(user) {
                if(!user) {
                    return res.badRequest("The user " + userId + " does not exist");
                }
                ...
                return PluginsDao.findByIdOrName(pluginId)
                    .then(function(plugin) {
                        if(!plugin) {
                            return res.badRequest("The plugin " + pluginId + " does not exist");
                        }

                        // check module
                        if(!plugin.hasModule(moduleName)) {
                            return res.badRequest("The module " + moduleName + " does not exist for plugin " + plugin.name);
                        }
