...
class Module{
    ...
    initializeScreen(screenContainer, cb) {
        ...
        var router = require('express').Router();
        this.test = router;
        screenContainer.instance.initialize(app, router, function(err) {
            if(err) {
                return cb(err);
            }
            ...
            var staticMiddleware = express.static(screenContainer.packageConfig.publicPath);
            //self.stat.ScreenModuleName = screenContainer.moduleName;
            //console.log(self.stat);
            //function Salut(req, res, next) {
            //    return express.static(screenContainer.packageConfig.publicPath)(req, res, next);
            //};
            //Salut.TOTO = "sdf";
            app.use(routeNamespace, staticMiddleware);

            self.screenModulesRef.set(screenContainer.moduleName, { router: router, staticMiddleware: staticMiddleware });
    ...
    destroyScreen(screenContainer, cb) {
        ...
        screenContainer.instance.destroy(function(err) {
            ...
            self.app._router.stack = self.app._router.stack.filter(function(layer) {
                ...
                return (layer.handle !== self.screenModulesRef.get(screenContainer.moduleName).router)
                && (layer.handle !== self.screenModulesRef.get(screenContainer.moduleName).staticMiddleware);
            });
            self.screenModulesRef.delete(screenContainer.moduleName);
