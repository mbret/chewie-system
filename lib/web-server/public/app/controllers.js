...
angular.module('myBuddy')
    ...
    .controller('IndexController', function($scope, $http, toastr, APP_CONFIG, apiService, user){
    ...
    .controller('MessagesAdaptersListController', function($scope, $http, APP_CONFIG, apiService){
        $scope.adapters = [];
        apiService.get(APP_CONFIG.apiUri + '/messages-adapters', {}).then(function(data){
            $scope.adapters = data;
        }, function(err){
            console.error(err);
        });
    ...
    .controller('ModulesController', function($scope, $http, $uibModal, APP_CONFIG, $log, tasksService, notificationService){
        ...
        $scope.addInstantTask = function(size, module){
            var modalInstance = $uibModal.open({
                ...
                templateUrl: APP_CONFIG.viewBaseUrl + '/modals/add-instant-task.html',
        ...
        $scope.addScheduledTask = function (size, module) {
            ...
            var modalInstance = $uibModal.open({
                animation: $scope.animationsEnabled,
                templateUrl: APP_CONFIG.viewBaseUrl + '/modals/add-scheduled-task.html',
        ...
        $scope.addTriggerTask = function(size, module){
            var modalInstance = $uibModal.open({
                templateUrl: APP_CONFIG.viewBaseUrl + '/modals/task-triggers-select.html',
            ...
            modalInstance.result.then(function (taskTrigger) {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: APP_CONFIG.viewBaseUrl + '/modals/add-trigger-task.html',
        ...
        $scope.addMovementTriggeredTask = function(size, module){
            var modalInstance = $uibModal.open({
                animation: $scope.animationsEnabled,
                templateUrl: APP_CONFIG.viewBaseUrl + '/modals/add-movement-commanded-task.html',
    ...
    .controller('SpeechController', function($scope, apiService, annyang, APP_CONFIG, _, notificationService){
        ...
        // Load commands
        apiService.get(APP_CONFIG.apiUri + '/speech/commands').then(function(entries){
            _.forEach(entries, function(entry){
                console.log(entry);

                // Create a new command with its correct api call to execute
                commands[entry.command] = function(){
                   apiService.post(APP_CONFIG.apiUri + '/speech/commands', {command: entry.command});
                }
            });
    ...
    .controller('CoreModulesDetailConfigController', function($scope, $http, $uibModal, APP_CONFIG, $log, pluginsService, $stateParams){
