(function(){
    ...
    angular
        ...
        .controller('TasksController', function($rootScope, $scope, $http, APP_CONFIG, TASK_TYPE, notificationService, apiService, tasksService, _, SweetAlert, auth){
            ...
            apiService.get('/users/:id/task-modules'.replace(':id', auth.getUser().id)).then(function(data){
                $scope.tasks = data;
        ...
        .controller('CreateController', function($scope, $http, $uibModal, APP_CONFIG, $log, tasksService, notificationService, apiService, auth){
            ...
            apiService.get('/users/:id/task-modules'.replace(':id', auth.getUser().id)).then(function(response){
                $scope.modules = response;
            });
        })

        .controller('CreateFormController', function($scope, $stateParams, pluginsService, $timeout, tasksService, $state, notificationService, _, triggersService){
            $scope.module = null;
