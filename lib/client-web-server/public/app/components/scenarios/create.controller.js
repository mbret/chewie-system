(function(){
    ...
    angular
        ...
        .controller("CreateScenariosController", function ($scope, $uibModal, apiService, auth, util, _, notificationService) {
            ...
            // retrieve the tasks
            apiService.get(util.format('/users/%s/modules', auth.getUser().id), { type: "task" })
                .then(function(data){
        ...
        .controller("CreateScenariosNewItemController", function($scope, $uibModalInstance, _, id, $uibModal, elements, apiService, authenticationService, util) {
            ...
            $scope.trigger = function(plugin) {
                ...
                var triggers = _.filter($scope.triggers, function(trigger) {
                    return trigger.plugin.id === pluginId;
                });
            ...
            $scope.task = function(plugin) {
                var tasks = _.filter($scope.tasks, function(module) {
                    return module.plugin.id === plugin.id;
                });
                // Pick up a task
                var modal = $uibModal.open({
                    animation: true,
            ...
            // fetch tasks
            apiService.get(util.format('/users/%s/modules', authenticationService.getUser().id), { type: "task" })
                .then(function(data) {
                    $scope.tasks = data;
                    // build array of unique key
                    $scope.tasks.forEach(function(module) {
                        $scope.taskPlugins[module.plugin.id] = module.plugin;
                    });
         ...
         * Choose a task.
         */
        .controller("scenarios.create.ChooseTaskController", function($scope, apiService, util, _, authenticationService, $uibModalInstance, tasks) {
            $scope.modules = tasks;
