...
class RuntimeHelper {
    ...
    registerTask(task, cb){
        ...
        if(!this.system.modules.has(task.moduleId)){
            this.logger.warn('Unable to register %s [%s] because its module [%s] from plugin id:%s does not seems loaded', task.constructor.name, task.getId(), task.moduleName, task.pluginId);
            return cb(new Error("The module " + task.moduleName + " does not seems to be loaded. Please verify if user has registered the plugin id:" + task.pluginId));
        }

        // Keep task in collection
        // The task is not active yet, just stored.
        this.system.tasks.push(task); // @todo remove it
