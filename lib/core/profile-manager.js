...
class ProfileManager extends CustomEventEmitter{
    ...
    startProfile(userId){
        ...
        return (new Promise(function(resolve, reject){
            ...
            self.logger.debug('Profile %s starting', userId);
                ...
                .then(function(user){
                    ...
                    taskQueue.proceed('profile:start', null, function(errs){
                        if(errs){
                            ...
                            _.forEach(errs, self.logger.error);
                            return reject(new Error(txt));
                        }
                        setTimeout(function(){
                            self.lock = false;
                            self.emit('profile:start:complete');
                            self.logger.info('Profile %s started', userId);
                            return resolve();
                        }, 1);
                    });
                })
            ...
            .catch(function(err){
                self.logger.error('Error while starting profile %s', userId);
