...
class Module{
    ...
    initialize(cb){
        ...
        this.daemon.on('api-server:initialized', function(){
            async.series([
                ...
                // log user (for now)
                function(done){
                    self.daemon.getCurrentUser().logIn('admin', 'password', done);
                },
                ...
                function(done){
                    ...
                    app.use(lessMiddleware(__dirname + '/public', {
                        dest: path.join(MyBuddy.configHandler.getConfig().tmpDir + '/web-server'),
                    }));
                    app.use(express.static(__dirname + '/public'));

                    // Merge the public tmp folder with regular public for static assets.
                    app.use(express.static(MyBuddy.configHandler.getConfig().tmpDir + '/web-server'));
                    ...
                    self.server = http.createServer(app).listen(self.daemon.configHandler.getConfig().webServerPort, done);
                }
    ...
    _getGoogleOauthClient(){
        oauth2Client = new OAuth2(this.daemon.getCurrentUser().getConfig().externalServices.google.auth.clientId, this.daemon.getCurrentUser().getConfig().externalServices.google.auth.clientSecret, 'https://localhost:3000/auth/google/callback');
