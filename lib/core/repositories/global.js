    ...
    synchronize(plugins){
        ...
        return new Promise(function(resolve, reject){
            async.each(plugins, function(plugin, done){
                ...
                var pluginDir = self.system.localRepository.getPluginDir(plugin.id);//, function(err, dir){
                self.system.localRepository.pluginExist(pluginDir)
                    .then(function(exist) {
                        if(!exist) {
                            return done(new Error('Unable to synchronize plugin ' + plugin.id + ' because the plugin directory ' + pluginDir + ' does not seems to exist anymore'));
                        }
                        var dest = path.resolve(self.pluginsTmpDir, plugin.id);
