    ...
    synchronize(plugins){
        ...
        return new Promise(function(resolve, reject){
            async.each(plugins, function(plugin, done){
                ...
                self.system.localRepository.getPluginDir(plugin.id, function(err, dir){
                    if(err){
                       return done(err);
                    }
                    if(!dir){
                        return done(new Error('Unable to synchronize plugin ' + plugin.id + ' because the plugin directory does not seems to exist'));
                    }
