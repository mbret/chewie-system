...
class RuntimeHelper {
    ...
    executeTask(task){
        ...
        var myTask = new Task(this.system, task.id, undefined, task.userId, task.pluginId, task.moduleId, task.options);

        // attach it to system
        this.system.tasks.set(myTask.executionId, myTask);
        ...
        // emit event
        this.system.emit("runtime:task:new", myTask);
    }
}
