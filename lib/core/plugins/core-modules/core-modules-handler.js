...
class CoreModulesHandler{
    ...
    constructor(){
        this.logger = MyBuddy.logger.Logger.getLogger('CoreModulesHandler');
    }
            ...
            required.forEach(function(require){
                switch(require){
                    case 'daemon':
                        ...
                        break;
                    case 'logger':
                        args.push(MyBuddy.logger.Logger.getLogger('Module - ' + moduleName));
                        break;
                    case 'scheduler':
    ...
    initializeModules(modules, cb){
        ...
        async.each(modules, function(container, done){
                ...
                container.instance.initialize(function(err){
                    if(err){
                        self.logger.error('Error while starting core module [%s]', container.id);
                        return done(err);
                    }
                    self.logger.verbose('Core module [%s] started', container.id);
