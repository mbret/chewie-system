...
class PluginsHandler{
    ...
    loadPlugins(pluginsToLoad, done){

        var self = this;
        var plugins = [];
        var repository = path.resolve(this.system.getConfig().system.dataDir, 'plugins');

        async.each(pluginsToLoad, function(pluginToLoad, cb) {

            var moduleName = pluginToLoad.name;
            var modulePath = path.resolve(repository, moduleName);
            logger.debug('Load plugin [%s] in [%s]', moduleName, modulePath);
            try{
