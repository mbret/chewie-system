...
class PluginsHandler{
                ...
                Module.call(null, new PluginHelper(self.system, plugin), function(err) {
                    ...
                    async.each(pluginPackage.modules, function(moduleToRegister, cbModuleToRegister) {
                        switch(moduleToRegister.type) {
                            case "screen" :
                                self.registerModule(plugin, moduleToRegister.name, moduleToRegister, moduleToRegister.module, cbModuleToRegister);
                                break;
                            default:
    ...
    registerModule(pluginContainer, moduleName, modulePackageConfig, moduleClass, cb) {
        var self = this;
        ...
        // Extract user options for this module
        // The options may be null
        this.system.database.getAdapter('plugins').getUserOptions(pluginContainer.id, moduleName, "module", function(err, options){
            if(err){
                return cb(err);
            }
            ...
            var tmp = new Modules.Container(self.system, pluginContainer, moduleName, null, options, modulePackageConfig);
