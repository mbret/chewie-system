...
class PluginsHandler{
    ...
    loadPlugins(pluginsToLoad, done){
        ...
        var self = this;
        var plugins = [];
        var pluginDir = path.resolve(this.system.getConfig().system.dataDir, 'plugins');
        ...
        async.each(pluginsToLoad, function(pluginToLoad, cb) {
            ...
            if(!PluginsHandler.isPluginValid(Module)){
                ...
                self._registerPlugin(Module, moduleName, packageJson, pluginPackage, function(err, plugin){
                    if(err){
    ...
    _registerPlugin(Module, moduleName, packageJson, pluginPackage, cb){
        var self = this;
        ...
        sync.fiber(function(){
            ...
            var instance = Module.apply(null, [new PluginHelper(self.system, plugin)]);
