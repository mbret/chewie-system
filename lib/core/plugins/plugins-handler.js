...
class PluginsHandler{
        ...
        return new Promise(function(resolve, reject) {
            async.each(pluginsToLoad, function(pluginToLoad, cb) {
                ...
                if(!PluginsHandler.isPluginValid(Module)){
                    ...
                    self.logger.debug("Module %s is valid", moduleName);
     ...
     * @private
     */
    _registerPlugin(profileId, Plugin, moduleName, packageJson, pluginPackage, cb){
        var self = this;
        ...
        this.system.apiService.findPlugin(profileId, moduleName)
            .then(function(data){
                ...
                var plugin = new PluginContainer(data.get("id"), moduleName, packageJson, pluginPackage, options, {
                    tmpDir: pluginTmpDir,
                ...
                Plugin.call(null, new PluginHelper(self.system, plugin), function(err) {
                    ...
                    if(!self._isModulePackageConfigValid(pluginPackage.modules)) {
                        return cb(new Error("Your modules package config does not seems to be valid"));
                    }
                    ...
                    async.each(pluginPackage.modules, function(moduleToRegister, cbModuleToRegister) {
                        ...
                        //     case "screen" :
                                self.registerModule(profileId, plugin, moduleToRegister.name, moduleToRegister, moduleToRegister.module, cbModuleToRegister);
                                // break;
                            // default:
                            //     return cbModuleToRegister(new Error("Module not recognized"));
                        // }
                    }, function(err) {
     ...
     * @private
     */
    _isModulePackageConfigValid(modulesConfig) {
        var valid = true;
        modulesConfig.forEach(function(moduleConfig) {
            if(!moduleConfig.module || (!typeof moduleConfig.module === 'function')) {
                valid = false;
            }
        });
     ...
     * @returns {*}
     */
    registerModule(profileId, plugin, moduleName, modulePackageConfig, moduleClass, cb) {
        var self = this;
        ...
        this.system.apiService.getModule(profileId, plugin.id, moduleName)
            .then(function(module) {
                ...
                var tmp = new Modules.Container(self.system, plugin, moduleName, null, module.options, modulePackageConfig);
                ...
                console.log(tmp.id);
                // register and attach module to daemon
                self.system.modules.set(moduleName, tmp);

                self.logger.verbose('Module %s registered', moduleName);
            })
            .catch(cb);
    }
    ...
    //     console.log(this.system.modules.keys());
