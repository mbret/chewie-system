...
var MessageAdapterContainer = require(CORE_DIR + '/plugins/output-adapters/message-adapter.js');
var MessageAdapterHelper = require(CORE_DIR + '/plugins/output-adapters/message-adapter-helper.js');
...
class PluginsHandler{
    ...
    loadPlugins(done){
        ...
        var self = this;
        var repositories = this.system.configHandler.getConfig().plugins.localRepositories;
        var plugins = [];
        var pluginsToLoad = this.system.configHandler.getConfig().loadPlugins;
        ...
        async.each(pluginsToLoad, function(moduleName, cb) {
            ...
            if(!PluginsHandler.isPluginValid(Module)){
                ...
                try{
                    ...
                    var args = [moduleName, new PluginHelper(self.system, plugin)];
    ...
    registerCoreModule(pluginId, name, module, cb){
        ...
        this.system.database.getAdapter('plugins').getUserOptions(pluginId, name, 'core-module', function(err, options){
            if(err){
                return cb(err);
            }
    ...
    registerModule(pluginId, name, module, cb){
        ...
        this.system.database.getAdapter('plugins').getUserOptions(pluginId, name, 'task-module', function(err, options){
            if(err){
                return cb(err);
            }
