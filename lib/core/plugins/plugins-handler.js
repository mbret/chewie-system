...
class PluginsHandler{
    ...
    _registerPlugin(profileId, Plugin, moduleName, packageJson, pluginPackage, cb){
        ...
        this.system.apiService.findPlugin(profileId, moduleName)
            .then(function(data){
                ...
                var plugin = new PluginContainer(data.id, moduleName, packageJson, pluginPackage, options, {
                    tmpDir: pluginTmpDir,
                ...
                Plugin.call(null, new PluginHelper(self.system, plugin), function(err) {
                    ...
                    var errors = self.chekModulePackageConfig(pluginPackage.modules);
                    if(errors.length > 0) {
                    ...
                    async.each(pluginPackage.modules, function(moduleToRegister, cbModuleToRegister) {
                        ...
                        //     case "screen" :
                                self.loadModule(profileId, plugin, moduleToRegister.name, moduleToRegister, require(moduleToRegister.module), cbModuleToRegister);
                                // break;
                            // default:
     ...
     * @private
     */
    chekModulePackageConfig(modulesConfig) {
        var errors = [];
     ...
     * @returns {*}
     */
    loadModule(profileId, plugin, moduleName, modulePackageConfig, moduleClass, cb) {
        var self = this;
        ...
        if(modulePackageConfig.type === "task" && !(typeof moduleClass.prototype.newTask === 'function')){
            ...
            moduleClass.prototype.newTask = function(task) {  };
        }
        ...
        this.system.apiService.getModule(profileId, plugin.id, moduleName)
            .then(function(module) {
                ...
                self.system.modules.set(tmp.id, tmp);
