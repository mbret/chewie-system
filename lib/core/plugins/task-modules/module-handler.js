...
class ModuleHandler{
    ...
    registerNewTask(task, cb){
        ...
        if(!MyBuddy.pluginsHandler.hasTaskModule(task.module, task.pluginId)){
            logger.warn('Unable to register %s [%s] because its module [%s] from [%s] does not exist', task.constructor.name, task.id, task.module, task.pluginId);
            return;
        }
        ...
        logger.verbose('New %s [%s] registered for module [%s] from [%s]', task.constructor.name, task.id, task.module, task.pluginId);
