...
var DirectTaskTrigger = require(CORE_DIR + '/plugins/tasks/task-trigger-direct');
...
class ModuleHandler{
    ...
    saveUserTask(task, cb){
        ...
        var onlyDirect = true;
        _.forEach(task.getTriggers(), function(trigger){
            if(!(trigger instanceof DirectTaskTrigger)){
                onlyDirect = false;
            }
        });
        if(onlyDirect){
        ...
        this.system.database.saveTask(task, function(err, id){
            return cb(err);
        });
    }
}
