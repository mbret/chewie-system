...
class Task{
    ...
    constructor(system, id, moduleId, pluginId, options, triggers){
    ...
    _checkArguments(args){
        ...
        if(!args[3]) throw new Error('Plugin id invalid ' + args[3]);
    ...
    static Build(object){
        ...
        _.forEach(object.triggers, function(trigger){
            triggers.push(TaskTriggers.Build(trigger));
        });

        return new Task(object.system, object.id, object.moduleId, object.pluginId, object.options, triggers);
    }
