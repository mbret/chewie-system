...
class TaskTriggerTrigger extends TaskTriggerBase{
    ...
    initialize(){
        ...
        if(!this.system.pluginsHandler.hasTaskTrigger(this.trigger.id)){
            var message = sprintf('Task [%s]: Unable to initialize trigger [%s] because its trigger module [%s] does not exist', this.task.getId(), this.id, this.task.id, this.trigger.id);
            logger.warn(message);
