...
var logger  = MyBuddy.logger.Logger.getLogger('SystemPersistence');
...
class SystemPersistence extends BaseAdapter{
    ...
    loadUserConfig(cb){
        ...
        this.persistence.find(this.configsStorage, { userID: this.system.getCurrentProfile().getId() }, function(err, row){
    ...
    saveUserConfig(config, cb){
        ...
        var data = {
            userID: this.system.getCurrentProfile().getId(),
            config: config || {}
        };
