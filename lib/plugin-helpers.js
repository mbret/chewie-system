...
class PluginHelper extends AbstractHelper{
    ...
    registerCoreModule(moduleName, module){
        ...
        var coreModule = new CoreModule(moduleName, null);
        var helper = new CoreModuleHelper(MyBuddy, coreModule);
        coreModule.setInstance(new module(helper));

        // register global core module
        MyBuddy.coreModulesNew.push(coreModule);

        logger.verbose('Plugin [%s] Registered new core module [%s]', this.plugin.name, moduleName);
    }
}
