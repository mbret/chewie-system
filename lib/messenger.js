...
class Messenger{
     ...
     * @param adapter
     */
    registerMessageAdapter(name, adapter, config){
        var self = this;
        ...
        MessageAdapter.load(name, function(err, entry){
            ...
            var messageAdapter = new MessageAdapter(self.daemon, name, null, config, userConfig);
            var helper = new AdapterHelper(self.daemon, messageAdapter);
    ...
    initializeAdapters(cb){
        var self = this;
        async.forEachOf(this.daemon.messenger.getAdapters(),
