...
class Scheduler{
    ...
    saveSchedule(method, when, interval, timerObject){
        var self = this;
        // Save this schedule
        var scheduleProcess = new ScheduleProcess(this, method, when, interval, timerObject);
    ...
    subscribe(schedule, cb){
        ...
        if(schedule.method === "interval"){
            timerObject = self._subscribeInterval(schedule.interval, function(){
        ...
        // Moment schedule
            ...
            if(Array.isArray(schedule.when)){
                timerObject = self._subscribeMoment(schedule, cb);
            }
            // "12:00"
            else{
                timerObject = self._subscribeMoment(schedule, cb);
    ...
    _subscribeInterval(interval, cb){
        console.log("Requested schedule: interval of %s.", interval);
