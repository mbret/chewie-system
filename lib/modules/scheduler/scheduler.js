...
var RangeScheduleProcess = require('./range-process.js');
...
class Scheduler{
    ...
    subscribe(schedule, cb){
        ...
        // second cb called when we leave the range
            ...
            scheduleProcess = self._subscribeRange(schedule, cb);
    ...
    _subscribeInterval(schedule, cb){
        ...
        var nextTick = moment();
        nextTick.add(interval, 'ms');
        scheduleProcess.nextTick = nextTick;
    ...
    _subscribeMoment(schedule, cb){
        ...
        (function loop(acceptNowMoment) {
            ...
            if(numberTooLarge){
                console.log('Using temporized timeout while next tick is over MAXIMUM_x32. use [%s]', config.x32Bit_NUMBER_LIMIT);
     ...
     * @param schedule
     * @param cb
     * @returns {ScheduleProcess|exports|module.exports}
     * @private
     */
    _subscribeRange(schedule, cb){
        var rangeScheduleProcess = new RangeScheduleProcess(this, 'range', schedule, null);
        var format = schedule.range.format;
        var from = schedule.range.from;
        var to = schedule.range.to;

        // Get entered moment
        rangeScheduleProcess.fromProcess = this._subscribeMoment({
            when: [from, format]
        }, function(){
            return cb(new ScheduleEvent({enter: true}));
        });

        // Get left moment
        rangeScheduleProcess.toProcess = this._subscribeMoment({
            when: [to, format]
        }, function(){
            return cb(new ScheduleEvent({exit: true}));
        });

        // We handle the case of current day.
        // For example if it is 15h and the range is 13h -> 16h we have to test it
        // because the subscription will be triggered at 13h next day for enter event.
        var fromCurrentTick = moment(from, format);
        var toCurrentTick = moment(to, format);
        if(fromCurrentTick.isBefore(moment()) && moment().isBefore(toCurrentTick)){
            return cb(new ScheduleEvent({enter: true}));
        }

        return rangeScheduleProcess;
    }
}
