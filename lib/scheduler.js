...
class Scheduler{
    ...
    /**
     *
     * @param {ScheduleProcess} scheduleDaemon
     * @returns {*}
     */
    unSubscribe(scheduleDaemon){

        throw new Error('todo');
        if(Array.isArray(scheduleDaemon)){
            return this.unSubscribeMultiple(scheduleDaemon);
        }

        // clearTimeout handle both interval / timeout
        clearTimeout(scheduleDaemon);
    }
    ...
    unSubscribeMultiple(scheduleProcess){
        throw new Error('todo');
        var self = this;
        _.forEach(scheduleDaemons, function(daemon){
