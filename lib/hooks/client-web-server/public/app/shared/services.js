    ...
    .provider("apiService", function() {
        this.$get = function($http, $log, APP_CONFIG){
            ...
            function query(method, url, data) {
                $log.info("%s %s", method.toUpperCase(), APP_CONFIG.apiUrl + url, angular.toJson(data));
                return $http({
                    method: method,
                    url: url,
                    ...
                    $log.info("Response for %s %s", method.toUpperCase(), APP_CONFIG.apiUrl + url, response.data);
                    return response;
                });
            }
        }
