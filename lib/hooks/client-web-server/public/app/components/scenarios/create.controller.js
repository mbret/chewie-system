(function(){
    ...
    angular
         ...
         */
        .controller("CreateScenariosController", function ($scope, $uibModal, sharedApiService, auth, util, _, notificationService) {
            ...
            function configureTrigger(item) {
                var modal = $uibModal.open({
                    ...
                    resolve: {
                        ...
                        trigger: function() { return _.find($scope.triggers, {id: item.triggerId}); },
                    }
                });
            }
            ...
            function configureTask(item) {
                $uibModal.open({
                    ...
                    resolve: {
                        item: function() { return item; },
                        module: function() { return _.find($scope.tasks, {id: item.taskId}); },
                    }
                });
            }
            ...
            // retrieve the tasks
            sharedApiService.get(util.format('/users/%s/modules', auth.getUser().id), { type: "task" })
                .then(function(data){
         ...
         * Add a new item.
         */
        .controller("CreateScenariosNewItemController", function($scope, $uibModalInstance, _, id, $uibModal, elements, sharedApiService, authenticationService, util) {
            ...
            // fetch triggers
            sharedApiService.get(util.format('/users/%s/modules', authenticationService.getUser().id), { type: "trigger" })
                .then(function(data) {
                    $scope.triggers = data;
            ...
            // fetch tasks
            sharedApiService.get(util.format('/users/%s/modules', authenticationService.getUser().id), { type: "task" })
                .then(function(data) {
                    $scope.tasks = data;
                    // build array of unique key
