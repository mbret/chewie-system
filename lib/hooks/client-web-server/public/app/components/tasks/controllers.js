(function(){
    ...
    angular
        ...
        .controller('TasksListController', function($rootScope, $scope, $http, APP_CONFIG, TASK_TYPE, notificationService, apiService, tasksService, _, SweetAlert, auth, util, mySocket){

            // http://stackoverflow.com/questions/8211744/convert-time-interval-given-in-seconds-into-more-human-readable-form
            $scope.tasks = [];

            apiService.get(util.format("/runtime/executing-tasks"))
                .then(function(data) {
            ...
            $scope.stopTask = function(executinTask) {
                apiService.delete(util.format("/runtime/executing-tasks/%s", executinTask.id));
            };

            $scope.executeTask = function(task) {
                apiService.post(util.format("/runtime/tasks/%s", task.id));
            };

            $scope.$on("$destroy", function() {
                mySocket.removeListener("runtime:executing-tasks:update", onRuntimeExecutingTasksUpdate);
            });

            mySocket.on("runtime:executing-tasks:update", onRuntimeExecutingTasksUpdate);
