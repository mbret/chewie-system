...
var ScheduleProcess = require(LIB_DIR + '/scheduler/schedule-process.js');
...
class Scheduler{
    ...
    subscribe(schedule, cb, cb2){
        ...
        // second cb called when we leave the range
            ...
            logger.verbose("Requested schedule: now ");
            cb();
        }
    ...
    _subscribeInterval(interval, cb){
        logger.verbose("Requested schedule: interval of %s.", interval);
    ...
    _subscribeMoment(when, cb){
        ...
        logger.verbose("Requested schedule: moment for [moment:%s] [format:%s] [days:%s].", momentDate, format, restrictedDays.join(','));
        ...
        (function loop(acceptNowMoment) {
            ...
            if(numberTooLarge){
                ...
                if(waitFor < 0){
                    logger.verbose("Schedule moment is related to past. No schedule planned!");
                }
                else{
                    logger.verbose("Schedule moment will be call in %s seconds / %s minutes / %s h / %s d.", (waitFor / 1000).toFixed(2), (waitFor / 1000 / 60).toFixed(2), (waitFor / 1000 / 60 / 60).toFixed(2), (waitFor / 1000 / 60 / 60 / 24).toFixed(2));
                    scheduleDaemon = setTimeout(function() {
                        cb();
