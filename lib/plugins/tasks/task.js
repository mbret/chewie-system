...
class Task{
    ...
    static Build(task, type){
        ...
        switch(type){
            ...
            case 'trigger':
            case 'TriggeredTask':
                return new CommandedTask(task.id, task.module, task.messageAdapters, task.options, task.trigger, task.triggerOptions);
...
class CommandedTask extends Task{
    ...
    constructor(id, module, messageAdapters, options, trigger, triggerOptions){
        super(id, module, messageAdapters, options);

        if(!trigger || trigger === '' || trigger === undefined){
            var err = new Error('Invalid trigger ' + this.module);
            err.code = Task.ERROR_CODE.INVALID;
            throw err;
        }

        if(!trigger || trigger === '' || trigger === undefined){
            var err = new Error('Invalid trigger options' + this.module);
            err.code = Task.ERROR_CODE.INVALID;
            throw err;
        }
