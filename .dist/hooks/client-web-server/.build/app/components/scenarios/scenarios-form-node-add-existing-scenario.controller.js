(function(){
    ...
    function controller($scope, $uibModalInstance, _, item, $uibModal, sharedApiService, authenticationService, util, apiService, APP_CONFIG) {
        ...
        $scope.confirm = function(form) {
            // $setSubmitted needed because out of form scope
            form.$setSubmitted();
            if (form.$valid) {
        ...
        //         });
                $uibModalInstance.close($scope.scenarios.find((tmp) => tmp.id === parseInt($scope.formData.scenario)));
            }
        };

        sharedApiService.get("/api/devices/" + APP_CONFIG.systemId + "/scenarios")
